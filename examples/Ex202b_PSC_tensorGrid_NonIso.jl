...
module Ex202b_PSC_tensorGrid_NonIso
...
function main(;
    ...
    control.max_round = 5
    control.damp_initial = 0.5 #0.1
    control.damp_growth = 1.61 #1.21 # >= 1
    # new
    control.abstol = 1.0e-7             # added
    control.reltol = 1.0e-7             # added
    control.tol_round = 1.0e-7          # added
    control.Δu_opt = Inf                # added
    if test == false
        println("*** done\n")
    end
    ...
    inival_eq = ChargeTransport.unknowns(ctsys)
    inival_eq .= 0.0

    # Temperature initial guess (NonIsothermal)
    if data.temperatureModel == NonIsothermal
        inival_eq[data.index_T, :] .= 300.0
    end

    solution = equilibrium_solve!(ctsys, inival = inival_eq, control = control, vacancyEnergyCalculation = vacancyEnergyCalculation, nonlinear_steps = 30.0)
    inival = solution
