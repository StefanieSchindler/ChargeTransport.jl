...
module Ex102b_PIN_nodal_doping_NonIso
...
function main(; Plotter = nothing, verbose = false, test = false, unknown_storage = :sparse)
    ...
    @local_unitfactors μm cm s ns V K ps W J m
    ...
    if Plotter !== nothing
        vis = GridVisualizer(; Plotter, layout = (4, 3), size = (1550, 800))
    ...
    params.heatCapacity = 2.0e6 * J / (K * m^3) 
    ...
    if Plotter !== nothing # plot solution and IV curve
        ...
        plot_temperature!(vis[4, 1], ctsys, solution, "Temperature for applied voltage Δu = $(biasValues[end])"; plotGridpoints = true)
        plot_temperatureFlux!(vis[4, 2], ctsys, solution, "Temperature flux for applied voltage Δu = $(biasValues[end])"; plotGridpoints = true)
